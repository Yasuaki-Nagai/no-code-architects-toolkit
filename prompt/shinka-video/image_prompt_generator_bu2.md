...
- タイムラインの**始点、中間点、終点**を意識し、構図に変化をつけて物語の緩急を演出してください。
  - **序盤**: `Wide Shot` や `Full Shot` を使い、壮大な環境と、その中で生まれたばかりの小さな生命を対比させる。
...
### 3. アートスタイル
- **全体像**: 「現代の古生物学の教科書や博物館のジオラマ」のような、高品質で様式化された3Dモデル風のイラストを目指す。
- **ライティング**: 光源は「左上からの単一光源」を基本とし、柔らかく方向性のある光で、被写体の形状と質感を強調する繊細な影を落とす。
- **色彩**: 全体的に彩度を抑え、落ち着いたアースカラーを使用する。
...
- **不適切な内容の禁止**: 暴力的、攻撃的、その他不適切なコンテンツは一切生成・記述しない。
...
```json
{
  "name": { "ja": "原始生命体", "en": "Primordial Life" },
  "summary": "...",
  "imagePrompt": "Wide shot of a vast, primordial ocean under a hazy red sky. In the center, a tiny, glowing cluster of Primordial Life drifts near a hydrothermal vent. The scene emphasizes the immense, empty scale of the early Earth. The surrounding water is dark and slightly murky. Lighting is soft and directional from the upper left. The color palette is muted and earthy. The illustration is styled as a modern scientific diorama with a gentle depth of field."
}
```
...
```json
{
  "name": { "ja": "ティクターリク", "en": "Tiktaalik" },
  "summary": "...",
  "imagePrompt": "Medium shot of an adult Tiktaalik at the water's edge. The camera focuses on its upper body, clearly showing the strong, lobed fins pressing into the muddy bank in a pre-walking motion. Its head is angled toward the right. The background features a single, softly blurred primitive plant. Lighting is soft and directional from the upper left, highlighting the matte, scaly texture of its skin. The color palette is muted and earthy. The illustration is styled as a modern scientific diorama, with the subject in sharp focus and a gentle depth of field."
}
```
