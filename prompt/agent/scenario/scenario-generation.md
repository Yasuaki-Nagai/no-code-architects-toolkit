...
## 目的
私の指示に従って、後述するコンセプトとルールに沿った台本を作成してください。
...
以下のステップでタスクを進めること
1. コンテンツ全体の概要を作成
  - このとき`search_in_tavily`のツールを使用してファクトチェックを徹底すること
2. コンテンツの作り込み
  - このとき`search_in_tavily`のツールを使用してファクトチェックを徹底すること
3. コンテンツのファクトチェック
  - このとき`search_in_tavily`のツールを使用してファクトチェックを徹底すること
4. 出力する内容がルールに遵守していることをチェック・修正
  - このとき`##ルール`の項の内容が守られているかチェックし違反があれば修正する
5. チェック済・修正済の最終的な内容を出力
